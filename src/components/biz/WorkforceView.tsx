 ...
 * employés magasin — et déplie pour chacun le détail complet de son activité
 * sur la période :
 *
 ...
 *  • Magasin          → chaque session de travail et toutes ses ventes.
...
import {
  ...
  Fuel, Store, Target, Gauge, Clock, ShoppingCart, Lock,
...
function WorkerCard({ w }: { w: WorkforceWorker; key?: React.Key }) {
  ...
  const netToPay = Math.max(0, w.salaryAmount - w.acomptesUnpaid - w.absencesTotal);
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
            ...
            <div className="px-4 pb-5 pt-1 space-y-6 border-t border-slate-100">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-2 pt-4">
                ...
                <Chip label="Salaire de base" value={money(w.salaryAmount)} />
              ...
              {w.brigades.length === 0 && w.sessions.length === 0 && w.sales.length === 0 && (
...
export default function WorkforceView({ report }: { report: WorkforceReport }) {
  ...
  const shown = useMemo(() => report.workers.filter(w => {
    ...
    const hasActivity = w.brigadesCount + w.sessionsCount + w.salesCount > 0;
    return matchQ
    ...
    <div className="space-y-8">
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full border-collapse">
              <thead><tr>
                ...
                <th className="table-head text-right">Activité</th>
              </tr></thead>
              <tbody>
                {report.parts.map(p => (
      ...
      <div className="card-glass p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {(['all', 'carburant', 'magasin'] as const).map(k => (
      ...
      <div className="space-y-2">
        ...
        <p className="text-[11px] text-slate-400 italic">
          Cliquez un employé pour dérouler ses brigades, sessions, ventes, salaires, acomptes et absences.
        </p>
      </div>
    </div>
  );
