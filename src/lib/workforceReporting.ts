 ...
 * Turns the whole application state (the Supabase fuel-station data + the
 * Magasin part) into ONE list of employees, each carrying every detail of what
 ...
 *  • Employés Magasin → leurs sessions de travail (ouverture, fermeture, fond de
 ...
 */
import {
  BizState, ModuleKey, MODULES, BizWorker, BizSession, BizSale,
  isReversedSale, netCashOfSale,
} from './bizConfig';
import { within } from './bizReporting';

export type WorkforcePart = 'carburant' | 'magasin';
...
export const PART_META: Record<WorkforcePart, { label: string; emoji: string; color: string }> = {
  ...
  magasin: { label: 'Magasin', emoji: '🏪', color: '#0e7490' },
};

// ─── Detail rows ─────────────────────────────────────────────────────────────
export interface WFPayment { id: string; label: string; date: string; amount: number; description?: string; mode?: string }
...
export interface WorkforceWorker {
  ...
  payMode: string; salaryType: string; salaryAmount: number;
...
const SALARY_LABEL: Record<string, string> = { mois: 'Mensuel', jour: 'Journalier' };

// ─── Magasin employees ───────────────────────────────────────────────────────
function bizWorker(
  w: BizWorker, key: ModuleKey, from: string, to: string,
  sessions: BizSession[], sales: BizSale[],
): WorkforceWorker {
  ...
  return {
    ...
    payMode: SALARY_LABEL[w.salaryType] || w.salaryType,
    salaryType: w.salaryType, salaryAmount: num(w.salaryAmount),
 ...
 *
 * C'est ce que lit la Caisse de la partie : l'écran où l'on paie les employés
 * doit montrer le travail sur lequel ils sont payés, et il ne peut pas le
 * compter autrement que le rapport.
 */
export function computeBizWorkforce(
  st: { workers?: BizWorker[]; sessions?: BizSession[]; sales?: BizSale[] },
  key: ModuleKey, from: string, to: string,
): WorkforceWorker[] {
  return (st.workers || [])
    .map(w => bizWorker(w, key, from, to, st.sessions || [], st.sales || []))
